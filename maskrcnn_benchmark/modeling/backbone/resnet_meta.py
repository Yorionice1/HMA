...
class ResNet_meta(MetaModule):
    ...
    def _freeze_backbone(self, freeze_at):
        ...
        for stage_index in range(freeze_at):
            print(stage_index)
            if stage_index == 0:
                m = self.features.init_block  # stage 0 is the stem
            else:
                m = getattr(self.features, "stage" + str(stage_index))
    ...
    def forward(self, x, params=None):
        if self.mode == 'maml':
            ...
            stem_features = self.features.init_block(x,params=self.get_subdict(params, 'backbone.features.init_block'))
            stage1_features = self.features.stage1(stem_features,params=self.get_subdict(params, 'backbone.features.stage1'))
            stage2_features = self.features.stage2(stage1_features,params=self.get_subdict(params, 'backbone.features.stage2'))
            stage3_features = self.features.stage3(stage2_features,params=self.get_subdict(params, 'backbone.features.stage3'))
            features = [stage1_features,stage2_features,stage3_features]
            
        else:
            stem_features = self.features.init_block(x)
            stage1_features = self.features.stage1(stem_features)
            stage2_features = self.features.stage2(stage1_features)
            stage3_features = self.features.stage3(stage2_features)
            features = [stage1_features,stage2_features,stage3_features]
            
            
        return features
    # def roi_forward(self, x, params=None):
    #     if self.mode == 'maml':
            
    #         stage4_features = self.features.stage4(x,params=self.get_subdict(params, 'stage4'))
